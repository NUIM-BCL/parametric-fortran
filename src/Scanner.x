...
-- OBSOLETE @stop_param_start = "!" $white* "{"
...
tokens :-
  ...
  "&"						;
  "!".*$					;
-- OBSOLETE  @stop_param_start			{ \s -> StopParamStart }
-- OBSOLETE  \!						{ \s -> Bang }
  "%"						{ \s -> Percent }
  "{"						{ \s -> LBrace }
  "}"						{ \s -> RBrace }
  "else" @line_space "if"   { \s -> Key "elseif" }
  @name						{ \s -> if elem (lower s) keywords then Key (lower s)
									   else ID s  }
...
--removeSpaces (x:xs) | x >= 'a' && x <= 'z' = (toUpper x):(removeSpaces xs)
--removeSpaces (x:xs) = x:(removeSpaces xs)

lower :: String -> String
lower = map toLower
--lower [] = []
--lower (x:xs) | x >= 'a' && x <= 'z' = (toLower x):(lower xs)
--lower (x:xs) = x:(lower xs)

--scan = alexScanTokens . removeSpaces
--scan = alexScanTokens . lower

--scanF :: String -> IO ()
--scanF f = readFile f >>= print . scan
