...
-----------------------------------------------------------
--
-- Soft line breaks modified for Fortran continuation
--
module PPrint 
        ( Doc
        , Pretty, pretty
...
softline        = group (SoftLine False)
softbreak       = group (SoftLine True)
...
data Doc        = Empty
                ...
                | SoftLine !Bool        -- True <=> when undone by group, do not insert a space 
                | Cat Doc Doc
                | Nest !Int Doc
...
flatten (SoftLine break) = if break then Empty else Text 1 " "
    ...
    where
      ...
      best n k (Cons i d ds)  
        = case d of
            ...
            SoftLine _  -> SContLine i (best i i ds)                 
...
renderCompact x   
    ...
    where
      ...
      scan k (d:ds) = case d of
                        ...
                        SoftLine _  -> SContLine 0 (scan 0 ds)    
...
displayS (SContLine i x)    = showString (' ':'&':'\n':indentation i) . displayS x
...
displayIO handle simpleDoc
    ...
    where
      ...
      display (SContLine i x) = do{ hPutStr handle (' ':'&':'\n':indentation i); display x}
...
instance Show Doc where
  showsPrec d doc       = displayS (renderPretty 0.8 75 doc)

putDoc :: Doc -> IO ()
putDoc doc              = hPutDoc stdout doc

hPutDoc :: Handle -> Doc -> IO ()
hPutDoc handle doc      = displayIO handle (renderPretty 0.8 75 doc)
